...
/* 自定义全局对象函数   */
	...
	var getInfo=function (fn,code,type){
	code = code || "";
	var data={
		PageId:code
	}
	...
	var tableclosdata=function (fn){
		...
		// debugger
			...
			utool.ajax.ajaxRequest("POST", config.path + "tableclos/list", postData, "json",false).done(function (returnData) {
                    returnData = JSON.parse(returnData);
                    if (returnData.Code == 200) {
                        fn && fn(returnData.Data)
                    }else{
                        returnData.Data={};
                        fn && fn(returnData)
					}
...
utool={
	...
	ajax : {
		ajaxRequest : function(method, url, data, datatype,async){
			async = async || true;
