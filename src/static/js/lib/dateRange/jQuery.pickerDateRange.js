...
function pickerDateRange(inputId, options) {
    ...
    //如果单选日期，不显示日期快捷按钮,确定与取消按钮，并且自动提交
    if(this.mOpts.isSingleDay) {
        this.mOpts.shortbtn = 0;
        this.mOpts.shortOpr=true;
        this.mOpts.autoSubmit=true;
    }
    //默认日历参数最大是3
    ...
    // 上一年的按钮
    this.preYear = 'dateRangePreYear_' + suffix;
    // 下一年的按钮
    this.nextYear = 'dateRangeNextYear_' + suffix;
    ...
    var wrapper = {
        ...
        ta: [
            ...
            '<div class="ta_calendar_cont cf" id="' + this.dateListId + '"' + (this.mOpts.calendars != 1 ? 'style="width:506px;"' : '') + '>',
            ...
            (this.mOpts.shortbtn ? '<div style="padding-bottom:10px; text-align: center;"> <div class="short-btn" id="aYesterday">昨天</div> <div class="short-btn" id="aToday">今天</div> <div class="short-btn" id="aRecent7Days">最近一周</div> <div class="short-btn" id="aRecent14Days">最近半个月</div><div class="short-btn" id="aRecent30Days">最近一个月</div><div class="short-btn" id="aRecent90Days">最近三个月</div></div>' : '') + '<div><div class="frm_msg">',
...
pickerDateRange.prototype.init = function (isCompare) {
    ...
    $('#' + this.preYear).bind('click', function () {
       __method.calendar_endDate.setMonth(__method.calendar_endDate.getMonth() - 12, 1);
       __method.mOpts.endDate = __method.date2ymd(__method.calendar_endDate).join('-');
       __method.init(isCompare);
       if (1 == __method.mOpts.calendars) {
           if ('' == $('#' + __method.startDateId).val()) {
...
pickerDateRange.prototype.addCSS = function (isCompare, specialClass) {
    ...
    if (this.mOpts.theme == 'ta') {
        ...
        //如果单日历
        if (this.mOpts.isSingleDay) {
            var $time = sDate.getTime() ? sDate.getTime() : eDate.getTime();
            if (isNaN($time)) {
                $time = new Date().getTime();
            }
            $('#' + this.calendarId + '_' + this.date2ymd(new Date($time)).join('-')).removeClass().addClass(this.mOpts.clickCss);
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    $("#" + this.calendarId).css('top', pos.top + 'px');
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    if (isTaTheme) {
        ...
        if (!this.mOpts.magicSelect) {
            if (0 == index) {
                $(td).append('<a href="javascript:void(0);" id="' + this.nextMonth + '"><i class="i_next" title="下一月"></i></a><a href="javascript:void(0);" id="' + this.nextYear + '"><i class="y_next"  title="下一年"></i></a>');
            }
            // 如果是第一个月的日期，则加上上一个月的链接
            if (index + 1 == this.mOpts.calendars) {
                $(td).append('<a href="javascript:void(0);" id="' + this.preMonth + '"><i class="i_pre" title="上一月"></i></a><a href="javascript:void(0);" id="' + this.preYear + '"><i class="y_pre" title="上一年"></i></a>');
            }
        }
    ...
    for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if (d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if ((this.mOpts.stopTodayBefore == true && (today.getTime() - d.getTime()) / 3600000 > 24) || (this.mOpts.stopToday == true && d.getTime() > today.getTime()) || d.getTime() < __method.mOpts.minValidDate * 1000 || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate * 1000)) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
