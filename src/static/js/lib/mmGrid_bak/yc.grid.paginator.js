...
tableclosdata(function (tableclosDataAll) {
    !function($) {
        var MMGrid = function(element, options) {
            ...
            if(browser.mobile|| browser.ios||browser.android
                ...
                ||browser.webApp){
                options.niceScroll = false;
            }

            if(options.niceScroll){
                //cursorcolor :光标颜色
                //autohidemode:滚动条，是否自动显示和隐藏
                //cursorwidth ：光标宽度
        ...
        MMGrid.prototype = {
            ...
            _initLayout: function($el) {
                ...
                var mmGrid = [
                    ...
                    '<a class="mmg-btnBackboardUp-copy">保存设置</a>',
                    '<a class="mmg-btnBackboardUp">关闭自定义</a>',
                    '</div>',
                    '<div class="mmg-bodyWrapper"></div>',
                    '<a class="mmg-btnBackboardDn">列表自定义</a>',
                ...
                if (browser.isIE6 && (!opts.height || opts.height === 'auto')) {
                    ...
                    if (opts.height === "auto") {
                        $mmGrid.height('100%');
                        var h=window.innerHeight - $mmGrid.offset().top - opts.height_offset;
                        $mmGrid.height(h > opts.min_height ? h : opts.min_height);
                    } else {
                        $mmGrid.height(opts.height);
                    }
                }
                ...
                if (opts.checkCol) {
                    var chkHtml = opts.multiSelect ? '<input type="checkbox" class="checkAll" >'
                        : '';//'<input type="checkbox" disabled="disabled" class="checkAll">';  // 注：如果check不是多选时 标题行不显示checkbox
                    opts.cols.unshift({
                        title: chkHtml, name: "multiSelect", width: 30, align: 'center', lockWidth: true, checkCol: true, renderer: function () {
                ...
                if (opts.indexCol) {
                    opts.cols.unshift({
                        title: '#', name: "indexCol", width: opts.indexColWidth, align: 'center', lockWidth: true, indexCol: true, renderer: function (val, item, rowIndex) {
            ...
            ,
            _expandCols: function(cols,type) {
                var newCols = [];
                if (!cols) {
                    return newCols;
                }
				for (var colIndex = 0; colIndex < cols.length; colIndex++) {
                        ...
                        if (col.cols) {
                            newCols.push(col);
                            newCols.push.apply(newCols, this._expandCols(col.cols, 2));
                        } else {
                            newCols.push(col);
                        }
                    }
            ...
            _leafCols: function() {
                var opts = this.opts;
                var newCols = [];
                var cols = this._expandCols(opts.cols,3);
                for (var colIndex = 0; colIndex < cols.length; colIndex++) {
                    var col = cols[colIndex];
                    if (!col.cols) {
                        newCols.push(col);
            ...
            ,
            //获取cookie 记忆的列
            _getcookieCols: function() {
                var opts = this.opts;
                var newcookieCols = cookie.getCookie(opts.cookiekey);
                //如果cookie 没有值 则取所有列
                if (!newcookieCols) {
                    newcookieCols = {};
                    var cols = this._expandCols(opts.cols,4);
                    for (var colIndex = 0; colIndex < cols.length; colIndex++) {
                        var col = cols[colIndex];
                        if (!col.cols) {
                            newcookieCols[col.name] = true;
                        }
                    }
                } else {
                    newcookieCols = JSON.parse(newcookieCols);
                }
                return newcookieCols;
            }
            ,
            _expandThs: function() {
            ...
            _titleHtml: function(col, rowspan) {
                ...
                if (!col.cols) {
                    ...
                    var colIndex = $.inArray(col, this._expandCols(opts.cols,5));
                    ...
                    if(col.sortable){
                        titleHtml.push('<div class="mmg-sort mmg-defult"></div>');
                    }
                    ...
                    titleHtml.push('<th class="');
                    var colIndex = $.inArray(col, this._expandCols(opts.cols,6));
                    titleHtml.push(this._genColClass(colIndex));
            ...
            _initHead: function() {
                ...
                var expandCols = this._expandCols(opts.cols,7);
                $.each($ths, function(index) {
                    if (!expandCols[index].width) {
                        expandCols[index].width = 50;
                    }
            ...
            _initOptions: function() {
                ...
                if (cols) {
                    var bbHtml = ['<h1 class="mmg-stop">显示列</h1>'];
                    for (var colIndex = 0; colIndex < cols.length; colIndex++) {
                        bbHtml.push('<label class="mmg-stop" ');
                        if (cols[colIndex].checkCol || cols[colIndex].indexCol) {
                            bbHtml.push('style="display:none;" ');
                        }
                        var col = cols[colIndex];
                        bbHtml.push('><input class="mmg-stop" type="checkbox"  ');
                        var isHidden = false;
                        if(tableclosDataAll && tableclosDataAll.reqEntity && tableclosDataAll.reqEntity.length > 0) {
                            for (var j = 0; j < tableclosDataAll.reqEntity.length; j++) {
                                if (tableclosDataAll.reqEntity[j].name == col.name) {
                                    if (tableclosDataAll.reqEntity[j].hidden == "true") {
                                        isHidden = true;
                                    }
                                    break;
                                }
                        ...
                        bbHtml.push('/><span class="mmg-stop">');
                        if (col.title) {
            ...
            _initEvents: function() {
                ...
                var $backboard = this.$backboard;
                var $ths = this._expandThs();
                var expandCols = this._expandCols(opts.cols,8);
                var leafCols = this._leafCols();
                //获取记忆的列
                var cookiecols = this._getcookieCols();
                ...
                //保存设置
                $mmGrid.find('a.mmg-btnBackboardUp-copy').on('click', function() {
                    var curUrl = $(this)[0].baseURI;
                    curUrl = curUrl.substring(curUrl.lastIndexOf("/")+1,curUrl.length)
                    if(curUrl.indexOf("?")>=0){
                        curUrl =curUrl.substring(0,curUrl.indexOf("?"))
                    }
                    var saveData=[];
                    var colsMsg = ""
                    if(leafCols && leafCols.length > 0){
                        for(var i =0;i< leafCols.length;i++){
                            var nowShow =  leafCols[i].hidden?leafCols[i].hidden:"false";
                            var detailData={
                                name:leafCols[i].name,
                                hidden:nowShow
                            }
                            saveData.push(detailData)
                        }
                        var postData={
                            PageUrl:curUrl,
                            ReqEntity:saveData
                        ...
                        utool.ajax.ajaxRequest("POST",config.path + "tableclos/add",postData,"json").done(function (returnData){
                            ...
                            if(returnData.Code==200){
                                ...
                                layer.alert('操作失败:'+ returnData.Msg,{title : '错误',icon : 2});
                            }
                        })
                    }
                    $backboard.slideUp().queue(function(next) {
                ...
                $backboard.on('click', ':checkbox', function() {
                    ...
                    //region 列表记忆
                    if (opts.cookiekey) {
                        cookiecols[col.name] = col.hidden ? false : true;
                        cookie.addCookie(opts.cookiekey, JSON.stringify(cookiecols), opts.cookieHour);
                    }
                    //endregion
                });
                ...
                $head.on('click', '.mmg-title', function() {
                    ...
                    if (opts.url && opts.remoteSort) {
                        //that.load()
                        that.opts.sortChange.call(that,{sortName:col.name, sortStatus: sortStatus})
                    } else {
            ...
            ,
            //设置 记忆的列
            _setCookieCols: function() {
                var cookiecols = this._getcookieCols();
                var cols = this._leafCols();
                if (cols) {
                    for (var colIndex = 0; colIndex < cols.length; colIndex++) {
                        var col = cols[colIndex];
                        // 如果列名称是 checkbox列或序号列时不参与记忆，始终显示
                        if (col.name == "multiSelect" || col.name == "indexCol") {
                            col.hidden = false;
                        } else {
                            if (cookiecols[col.name]) {
                                col.hidden = false;
                            } else {
                                col.hidden = true;
                            }
            ...
            _rowHtml: function(item, rowIndex) {
                var opts = this.opts;
                var expandCols = this._expandCols(opts.cols,9);
                var leafCols = this._leafCols();
                ...
                if ($.isPlainObject(item)) {
                    ...
                    for (var colIndex = 0; colIndex < leafCols.length; colIndex++) {
                        ...
                        /** 修改 添加 title 提示,自定义class 2016年7月26日09:38:39
                         trHtml.push('">');
                         if (col.renderer) {
                        ...
                        //2016年7月26日 添加自定义行class
                        if (col.rowclass) {
                            switch (col.rowclass.constructor) {
                                case Function:
                                    //如果是方法
                                    trHtml.push(' ');
                                    trHtml.push(col.rowclass(item[col.name], item, rowIndex));
                                    break;
                                case String:
                                    //如果是字符串
                                    trHtml.push(' ');
                                    trHtml.push(col.rowclass);
                                    break;
                                default:
                                    break;
                            }
                        ...
                        //2016年7月26日 添加自定义 title
                        if (col.tips) {
                            switch (col.tips.constructor) {
                                case Function:
                                    trHtml.push(' title="' + col.tips(item[col.name], item, rowIndex) + '"');
                                    break;
                                case String:
                                    trHtml.push(' title="' + col.tips + '"');
                                    break;
                                case Boolean:
                                    if (col.renderer) {
                                        trHtml.push(' title="' + col.renderer(item[col.name], item, rowIndex) + '"');
                                    } else {
                                        trHtml.push(' title="' + item[col.name] + '"');
                                    }
                                    break;
                                default:
                                    break;
            ...
            ,
            _setColsWidth: function(clickType) {
                var all_this = this;
                var opts = this.opts;
                ...
                var $body = this.$body;
                var $ths = this._expandThs();
                var expandCols = this._expandCols(opts.cols,1);
                ...
                if(clickType == 1 || clickType == 2) {
                    ...
                    if(tableclosDataAll && tableclosDataAll .reqEntity && tableclosDataAll.reqEntity.length >0){
                        for (var colIndex = 0; colIndex < $ths.length; colIndex++) {
                            var $th = $ths.eq(colIndex);
                            styleText.push('.mmGrid .' + all_this._genColClass(colIndex) + ' {');
                            ...
                            // if(col.width){
                             //    styleText.push('width: ' + col.width + 'px;');
							// }else{
                             //    styleText.push('width: ' + width + 'px;');
							// }
                            var isHidden = false;
                    ...
                    utool.ajax.ajaxRequest("POST", config.path + "tableclos/list", postData, "json").done(function (returnData) {
                        returnData = JSON.parse(returnData);
                        var styleText = [];
                        if (returnData.Code == 200) {
                            for (var colIndex = 0; colIndex < $ths.length; colIndex++) {
                                ...
                                var isHidden = false;
                                for (var j = 0; j < returnData.Data.reqEntity.length; j++) {
                                    if (returnData.Data.reqEntity[j].name == col.name) {
                                        if (returnData.Data.reqEntity[j].hidden == "true") {
                                            isHidden = true;
                                        }
                                        break;
                                    }
                    ...
                    */
                    ...
                    var styleText = [];
                    var isRatio = 0;
                    if($ths && $ths.parent().length > 0){
                        var newWidth = $ths.parent().parent().parent().parent().parent()[0].clientWidth;
                        var countWidth = 0
                        for (var colIndex = 0; colIndex < $ths.length; colIndex++) {
                            var $th = $ths.eq(colIndex);
                            var width = $.data($th[0], 'col-width');
                            countWidth+=width;
                            if(countWidth>= newWidth){
                                if(colIndex < $ths.length-1){
                                    isRatio = newWidth/$ths.length
                                }
                                break;
                            }
                        }
                    }
                    // layer.msg("isRatio:"+isRatio)
                    for (var colIndex = 0; colIndex < $ths.length; colIndex++) {
                        var $th = $ths.eq(colIndex);
                        styleText.push('.mmGrid .' + all_this._genColClass(colIndex) + ' {');
                        var width = $.data($th[0], 'col-width');
                        ...
                        if(isRatio && isRatio>0) {
                            styleText.push('width: ' + isRatio + 'px;');
                        }else{
                            styleText.push('width: ' + width + 'px;');
                        }
                        // if(col.width){
            ...
            _fullWidthRows: function() {
                ...
                for (var i = 0; i < leafCol.length; i++) {
                    ...
                    if (!col.lockWidth && $th.is(':visible')) {
                        thsArr.push($th);
                    }else if(col.lockWidth && $th.is(':visible')){
                        lockClosWidth += col.width
                    }
                }
                // fitWidth = $mmGrid.width() -lockClosWidth;
            ...
            _loadAjax: function(args) {
                ...
                if (opts.remoteSort) {
                    ...
                    if (sortName) {
                        params.sortExpression = sortName;
                        params.sortDirection = sortStatus;
                    }
                }
                ...
                if(tokenUrl ){
                    var newUrl = opts.url+"?token="+tokenUrl;
                }
                ...
                $.ajax({
                    ...
                    try {
                        ...
                        if (data && $.isArray(data[opts.root])) {
                            data = $.extend({}, args, data); //$.extend(args, data) 会污染 args 对象
                        }
            ...
            load: function(args) {
                try {
                    ...
                    if(opts.multiSelect){
                        this.$mmGrid.find('.mmg-head .mmg-title input[type="checkbox"].checkAll').attr("checked",false);
                    }
            ...
            _resizeHeight: function() {
                ...
                if (opts.height === 'auto' && browser.isIE7) {
                    ...
                    //修改为计算高度
                    $bodyWrapper.height(window.innerHeight - $mmGrid.offset().top - $headWrapper.outerHeight(true) - opts.height_offset);
        ...
        $.fn.mmGrid.defaults = {
            ...
            height: 'auto' //表格高
            ,
            min_height: 250  // 表格自适应高度时最小高度
            ,
            height_offset:40 //页面高度计算偏移量
            ,
            cookiekey: '' //cookie 记忆列
            ,
            cookieHour: 0 //cookie 记忆时间（单位小时）
            ,
            cols: [] //数据模型
            ,
            url: false //AJAX请求数据的地址
            ,
            params: {} //AJAX请求的参数
            ,
            ...
            cache: false //AJAX缓存
            ,
            root: 'items' //指定json对象的数组字段为数据
            ,
            items: [] //数据使用本地对象数组
            ,
            autoLoad: true //是否表格准备好时加载数据
            ,
            remoteSort: true //是否使用服务器端排序
            ,
            sortName: '' //排序的字段名
            ,
            sortStatus: 'asc' //排序方向
            ...
            ,
            fullWidthRows: false //表格第一次加载数据时列伸展，自动充满表格
            ,
            nowrap: true //表格显示的数据超出列宽时是否换行
            ,
            showBackboard: true //是否显示选项背板。
            ,
            backboardMinHeight: 125 //选项背板最小高度
            ,
            niceScroll:true
            ,
    ...
    !function($) {
        ...
        MMPaginator.prototype = {
            _initLayout: function() {
                ...
                $limitList.on('change', function(limit) {
                    $el.data('page', 1);
                    limit=this.options[this.selectedIndex].value||"";
                    that.$mmGrid.opts.limitChange.call(that.$mmGrid,limit)
                });
            ...
            _plain: function(page, totalCount, limit) {
                ...
                //
                var $prev = $('<li class="prev"><a style="font-family: Arial;"> 上一页 </a></li>');
                if (page <= 1) {
                    ...
                    $prev.find('a').on('click', function() {
                        $el.data('page', page - 1);
                        //that.$mmGrid.load();
                        that.$mmGrid.opts.pageChange.call(that.$mmGrid,page-1);
                    });
                }
                ...
                $.each(list, function(index, item) {
                    ...
                    if (item === '...') {
                        ...
                        $li.find('a').text(item).prop('title', '第' + item + '页').on('click', function(e) {
                            $el.data('page', item);
                            //that.$mmGrid.load();
                            that.$mmGrid.opts.pageChange.call(that.$mmGrid,item);
                        });
                    }
                    $pageList.append($li);
                });
                //
                var $next = $('<li class="next"><a title="下一页" style="font-family: Arial;"> 下一页 </a></li>');
                if (page >= totalPage) {
            ...
            init: function($grid) {
                ...
                var params = {};
                params[opts.totalCountName] =opts.totalCount|| 0;
