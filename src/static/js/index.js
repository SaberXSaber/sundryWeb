    ...
    var index_module = new Vue({
        ...
        methods:{
            ...
            lockPage:function () {
                layer.open({
                    ...
                    closeBtn:0,
                    area: ['380px', '200px'],
                    content: ['locksure.html?userId=','no']
                });
            },
            updatepwd:function () {
                layer.open({
            ...
            refresh:function () {
                ...
                utool.ajax.ajaxRequest("POST", config.path + "getOperator" , {}, "json").done(function (returnData) {
                    ...
                    if (returnData.code == 200) {
                        index_module.UserName = returnData.msg;
                    }
                })
            },
        },
        created:function (){
           ...
           /* utool.ajax.ajaxRequest("POST", config.path + "getOperator" , {}, "json").done(function (returnData) {
            ...
            /*utool.ajax.ajaxRequest("POST",config.path + "getlogin" ,{},"json").done(function (returnData){
                returnData = JSON.parse(returnData);
                if(returnData.Code==200){
                    index_module.demoList = returnData.Rights;
                    index_module.UserName = returnData.UserName;
                    index_module.$nextTick(function (){
                        $(".J_menuItem").each(function (k) {
                            if (!$(this).attr("data-index")) {
                                $(this).attr("data-index", k)
                            }
