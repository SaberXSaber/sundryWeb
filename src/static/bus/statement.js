...
(function(){
			...
			methods : {
				...
				parentFn:function (name){
					layer.open({
						...
						closeBtn:false,
						area: ["100%", "100%"],
						content: './useadminchild.html?name=' + name,
						end:function (){
	...
	//选择用户
	var UseManngerChild= function(fn){
		var mmg=null;
		...
		mmg.on('cellSelected', function(e, item, rowIndex, colIndex){
			parent.$("#"+name).val(item.oName);
			parent.$("#"+name).attr("data-id",item.operatorId);
			var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
			parent.layer.close(index);
            fn && fn(item)
		})
	...
	BaseJs.UseManngerChild=UseManngerChild;
